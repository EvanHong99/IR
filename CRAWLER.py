        ...
        while True:
            try:
                ...
                if url not in list(used_url.values[:,0]):
                    page = get_page(url[0], config.HEADERS)
                    if not page:
                        logger.warning(url[0]+"  is empty")
                        time.sleep(3)
                        continue
                    ...
                    str_df=pd.DataFrame([[url[0],strings]],columns=['base_url','keywords'])

                    # 将链接关系（包括锚文本）写入csv文件
                    df=pd.DataFrame(columns=['base url','hook','linked url'])
                    for item in soup.select('a'):
                        href=item.get('href')
                        if href and href[:4]=='http':
                            df=df.append(pd.DataFrame([[url[0],item.string,href]],columns=['base url','hook','linked url']),ignore_index=True)
                            #若该url未被捕获过，那么将它加入到待爬取列表。若程序终止那么可能导致该页面捕获的所有url未能存入磁盘
                            if href not in used_url:
                                unused_url.append([href])
                    ...
                    threadLock1.acquire()
                    str_df.to_csv("pages/_page_contents.csv",encoding='utf-8',index=False,mode='a',columns=['base_url','keywords'])
                    df.to_csv(r"pages/_links.csv",encoding='utf-8',index=False,mode='a',columns=['base url','hook','linked url'])
                    threadLock1.release()
                    logger.debug("write" + str(self.threadID) + url[0])
